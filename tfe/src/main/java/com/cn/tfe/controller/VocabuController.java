...
public class VocabuController {
    ...
    private void preHandle(Map<String,List<String>> requestParam, SynHandleData synHandleData){
        ...
        TransApi api = new TransApi(appid, securityKey);
        List<Vocabu> vocabus = new ArrayList<>(params.size());
        List<Future<Vocabu>> futures = new ArrayList<>(params.size());
        List<Future<Vocabu>> doneFutures = new ArrayList<>(params.size());
...
//            }
    ...
    private Iterable<String> cleaningWordData(List<String> source){
        ...
        String regexStr = "^[a-z][a-z|\\s]*[a-z]*$";

        for(String word:source){
            String lowRepl = word.toLowerCase().replaceAll("[^a-z|\\s]*","").trim();
            if(StringUtils.isEmpty(lowRepl) || !lowRepl.matches(regexStr)){
                count++;
